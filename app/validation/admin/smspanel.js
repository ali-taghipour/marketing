...
class smsPanelValidator {
  createOrUpadte() {
    return [
      ...
      body("priceForEachSMS").custom((value) => {
        if (!value) {
          throw new Error("قیمت برای هر پیامک نباید خالی باشد");
        } else if (Number(value) == 0 || Number(value) < 0) {
          throw new Error("قیمت برای هر پیامک نباید صفر یا عددی منفی باشد");
        } else {
          return true;
        }
      }),
...
module.exports = new smsPanelValidator();
